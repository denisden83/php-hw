...
//АУТОРИЗОВАННЫЙ ПОЛЬЗОВАТЕЛЬ
    ...
    if (isset($_GET['id'])) {
        // удаляем photo и запись photo в бд
        ...
        if (!file_exists('photos/' . $_GET['photo'])) {
            try {
                ...
                $photo = $pdo->prepare($query);
                $photo->execute(['id' => $_GET['id']]);
    ...
    // выводим данные
    try {
        $query = "SELECT * FROM users WHERE photo NOT LIKE ''";
        $usr = $pdo->query($query);
    } catch (PDOException $e) {
        echo "Ошибка выполнения запроса: " . $e->getMessage();
    }
    ...
    <div class="container">
        ...
        <table class="table table-bordered">
            ...
            <?php while ($user = $usr->fetch()) { ?>
                <tr>
                    <td><?php echo $user['photo'] ?></td>
                    <td><img class="photo" src="photos/<?php echo $user['photo'] ?>" alt=""></td>
                    <td>
                        <a href="<?php echo "?id=" . urlencode($user['id']) .
                            "&photo=" . urlencode($user['photo']) ?>">Удалить аватарку пользователя</a>
                    </td>
                </tr>
            <?php } ?>
        </table>
